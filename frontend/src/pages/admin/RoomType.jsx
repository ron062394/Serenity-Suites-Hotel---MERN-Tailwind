...
function RoomType() {
  ...
  const [newRoomType, setNewRoomType] = useState({ roomType: '', description: '', price: '', capacity: '', amenities: [], images: [] });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      fetchRoomTypes();
      setNewRoomType({ roomType: '', description: '', price: '', capacity: '', amenities: [], images: [] });
      setEditingId(null);
    } catch (error) {
          ...
          {roomTypes.map((roomType) => (
            <div key={roomType._id} className="bg-gray-100 rounded-lg p-4 shadow">
              <div className="flex justify-between items-start mb-2">
                <h4 className="text-lg font-semibold">{roomType.roomType}</h4>
                <div>
                  <button
              ...
              <div className="grid grid-cols-3 gap-2">
                {roomType.images.map((image, index) => (
                  <img key={index} src={image} alt={`${roomType.roomType} ${index + 1}`} className="w-full h-20 object-cover rounded" />
                ))}
              </div>
            </div>
          ))}
